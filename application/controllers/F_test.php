...
class F_test extends CI_Controller {
...
$pdf->SetTitle('Total');
$pdf->SetSubject('Total de Registros');
...
for ($x=0; $x < count($datos) ; $x++) {
    ...
    $rows = $rows . '<tr><td>' . $datos[$x]->nombre . '</td><td>'.$datos[$x]->entrada_normal.'</td><td>'.$datos[$x]->entrada_retardo.'</td><td>'.$datos[$x]->entrada_falta.'</td><td>'.$datos[$x]->salida_normal.'</td><td>'.$datos[$x]->salida_anticipada.'</td><td>'.$datos[$x]->no_presento.'</td></tr>';
...
<table border="0">
    <tr>
        ...
        <th align="center" width="50%">
            <span style="font-size: large;">TOTAL DE REGISTROS</span>
            <BR>
